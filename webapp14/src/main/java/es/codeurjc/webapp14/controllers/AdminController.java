...
import es.codeurjc.webapp14.model.UserClient;
...
public class AdminController {
    ...
    private List<UserClient> users = new ArrayList<>();  // Lista de usuarios

    // Constructor para inicializar algunos productos (puedes quitar esto si usas una base de datos)
    public AdminController() {
        products.add(new Product(8754676, "Abrigos", "Elegancia y calidez", 50.00, "Ropa de Invierno", 20,
            List.of(
                new Review("Usuario1", "Excelente calidad, muy recomendable.", 5,true,8754676),
                new Review("Usuario2", "Muy cómodo y abrigado.", 4,true,8754676),
                new Review("Usuario3", "El color es un poco distinto al de la foto.", 3,true,8754676)
            )
        ));

        products.add(new Product(6758492, "Sudaderas", "Comodidad y estilo", 35.00, "Sudadera", 0,
            List.of(
                new Review("Usuario4", "Excelente calidad y muy cómoda.", 5,true,6758492),
                new Review("Usuario5", "Es súper calentita y cómoda, perfecta para el invierno. La volvería a comprar.", 5,false,6758492),
                new Review("Usuario1", "Me gusta el diseño, pero la tela es un poco más delgada de lo que esperaba.", 4,true,6758492)
            )
        ));

        products.add(new Product(9856321, "Chaquetas", "Diseño elegante", 15.00, "Chaquetas", 30,
            List.of(
                new Review("Usuario2", "Me encantó el material, es muy suave y cómodo. Perfecta para el día a día.", 5,false,9856321),
                new Review("Usuario4", "El diseño es bonito, pero la talla me quedó un poco ajustada. Recomiendo pedir una talla más grande.", 4,false,9856321),
                new Review("Usuario5", "Después de un par de lavadas, los colores siguen intactos. ¡Buena calidad!", 5,false,9856321)
            )
        ));

        users.add(new UserClient(0763774,"Juan Pérez", "juanperez@email.com", "Calle Falsa 123, Madrid", 3, "/images/undraw_profile.svg"));
        users.add(new UserClient(8768342,"María López", "marialopez@email.com", "Av. Siempre Viva 742, Barcelona", 5, "/images/undraw_profile.svg"));
        users.add(new UserClient(6736822,"Carlos Ramírez", "carlosramirez@email.com", "C/ Gran Vía 123, Valencia", 8, "/images/undraw_profile.svg"));
        users.add(new UserClient(2754556,"Laura Martínez", "lauramartinez@email.com", "Paseo del Prado 56, Sevilla", 4, "/images/undraw_profile.svg"));
        users.add(new UserClient(4566756,"Pedro Gómez", "pedrogomez@email.com", "Av. de la Constitución 101, Bilbao", 6, "/images/undraw_profile.svg"));

    }

    @GetMapping("/admin/products")
    public String showProducts(Model model) {
        model.addAttribute("products", products);  // Añadir la lista de productos al modelo
...
public String addProduct(@RequestParam(required = false) String name,
    ...
    if (name == null || name.trim().isEmpty() ||
        ...
        redirectAttributes.addFlashAttribute("error", "Todos los campos deben estar completos y el precio debe ser mayor a 0.");
    ...
    Product newProduct = new Product(generateUniqueId(), name, description, price, category, stock, new ArrayList<>());
    ...
    public String addCategory(@RequestParam("categoryName") String categoryName, Model model) {
        ...
        model.addAttribute("categories", categories);
        return "redirect:/admin/products";  // Redirigir para actualizar la lista de categorías
    }
    ...
    // Método para generar un ID único (esto puedes adaptarlo según tu lógica de base de datos)
    private int generateUniqueId() {
        return (int) (Math.random() * 10000000);  // Ejemplo de generación de ID aleatorio
    }
...
public String getUsers(Model model) {
    ...
    for (UserReports userReports : userReportsList) {
        userReports.setReviewCount(userReports.getReviews().size());  // Agregar un campo de conteo
    }
